...
fn main() {
    ...
    if target_os == "windows" {
        ...
        if !pdfium_dest.exists() {
            ...
            let mut file = fs::File::create(&temp_tarball).expect("Failed to create temporary tgz file");
            std::io::copy(&mut response, &mut file).expect("Failed to write download stream to tgz file");
            ...
            for entry in archive.entries().expect("Failed to read tar archive entries") {
                ...
                if path.to_str() == Some("bin/pdfium.dll") {
                    ...
                    let mut dest_file = fs::File::create(&pdfium_dest).expect("Failed to create output pdfium.dll");
                    std::io::copy(&mut entry, &mut dest_file).expect("Failed to extract pdfium.dll content");
                }
            }
